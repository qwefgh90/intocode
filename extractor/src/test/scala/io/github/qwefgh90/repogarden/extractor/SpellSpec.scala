...
class SpellSpec extends FlatSpec with Matchers {
	val logger = Logger(classOf[SpellSpec])
		...
		matches.asScala.foreach(res => {
			println(s"Potential error occurs from ${res.getFromPos} to ${res.getToPos} (${sentence1.substring(res.getFromPos, res.getToPos)})" +
		...
		matches2.asScala.foreach(res => {
			println(s"Potential error occurs from ${res.getFromPos} to ${res.getToPos} (${sentence2.substring(res.getFromPos, res.getToPos)})" +
					s"at line ${res.getEndLine()}, column ${res.getColumn()} ${res.getMessage()}")
					println("Suggested correction: " +
							res.getSuggestedReplacements());
